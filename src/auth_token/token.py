...
class TokenEncoder(Encoder):
    def __init__(self, algorithm: str, encrypt_key: str, decrypt_key: str):
        self.algorithm = _validate_algorithm(algorithm)
        self.encrypt_key = encrypt_key
        self.decrypt_key = decrypt_key

    def encode(self, payload: Dict) -> Token:
        return _encode(payload, self.algorithm, self.encrypt_key, self.decrypt_key)
...
def _encode(payload: Dict, algorithm: str, private_key: str, public_key: str) -> Token:
    _validate_algorithm(algorithm)
    expiration = datetime.datetime.utcnow() + datetime.timedelta(
        hours=DEFAULT_EXPIRATION
    )
...
def _decode(token: DecryptToken) -> Dict:
    ...
    if token.algorithm not in allowed:
        raise UnsupportedAlgorithm(
            f'incorrect crypto-algorithm: {token.algorithm} is not supported. try {allowed}'
        )
    try:
        return jwt.decode(token.value, token.decrypt_key, algorithms=[token.algorithm])
